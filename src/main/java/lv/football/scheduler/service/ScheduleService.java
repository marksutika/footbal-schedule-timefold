...
public class ScheduleService {
     ...
     * Variant values: "tabu", "lateAcceptance", "hillClimbing", "simulatedAnnealing", "constructionHeuristic" (or null for default per-type config)
     */
    public SchedulingSolution solveSchedule(String type, String variant, SchedulingSolution problem) {
        ...
        // Post-process construction heuristic solutions to remove trivial forbidden-date placements
        if (variant != null && (variant.equals("constructionHeuristic") || variant.equals("ch"))) {
            try {
    ...
    /**
     * Naive fixer: for matches landing on forbidden dates, try to move them to the nearest
     * timeslot/round that doesn't produce a forbidden date. This is a greedy post-processing
     * step (not optimal) intended to ensure the construction heuristic result respects H9.
     */
    private void fixForbiddenDates(SchedulingSolution solution) {
        ...
        if (rounds == null || rounds.isEmpty() || timeslots == null || timeslots.isEmpty()) return;

        // simple helper to compute match date for (round, timeslot)
        java.util.function.BiFunction<lv.football.scheduler.domain.Round, lv.football.scheduler.domain.Timeslot, java.time.LocalDate> dateFor = (r, t) -> {
            if (r == null || t == null) return null;
            return r.dateFor(t.getDayOfWeek());
        };

        for (var match : solution.getMatches()) {
            if (match.getRound() == null || match.getTimeslot() == null) continue;
...
private String configFileFor(String type, String variant) {
    ...
    */
    ...
    private String configFileFor(String type, String variant) {
        if (variant == null) return configFileForType(type);
